...
@section('content')
    ...
    <div class="container">
                    ...
                    <div class="modal-content">
                        ...
                        <form action="/admin-warga" method="POST">
                            ...
                            <div class="modal-body">
                                {{-- ID WALAS --}}
                                <input type="hidden" name="wali_kelas_id" value="{{ $wali_kelas->guru_id }}">
                                {{-- ID Kelas --}}
                                <input type="hidden" name="kelas_id" value="{{ $wali_kelas->kelas_id }}">
                                {{-- Siswa --}}
                                <div class="row">
                                    <div class="col mb-3">
                                        <label for="selectWalas" class="form-label">Siswa</label>
                                        <select class="form-control" id="selectWalas" name="siswa_id">
                                            @foreach ($siswa as $s)
                                                <option value="{{ $s->id }}">{{ $s->nomor_induk . ' - ' . $s->nama }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="row">
                                    <div class="col mb-3">
                                        ...
                                        <input type="text" class="form-control" name="guru"
                                            value="{{ $wali_kelas->guru->nama }}" readonly>
                                ...
                                <div class="row g-2">
                                    <div class="col mb-0">
                                        ...
                                        <input type="text" id="TahunPelajaran" class="form-control"
                                            placeholder="Ex : 2021/2022" name="tahun_pelajaran"
                                            value="{{ $wali_kelas->tahun_pelajaran }}" readonly>
                                    </div>
                                </div>
        ...
        <table id="example" class="table table-striped" style="width:100%">
            ...
            <tbody>
                @foreach ($warga_kelas as $w)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $w->siswa->nama }}</td>
                        <td>
                            <form action="/admin-warga/{{ $w->id }}" class="d-inline" method="POST">
                                @csrf
                                @method('DELETE')
